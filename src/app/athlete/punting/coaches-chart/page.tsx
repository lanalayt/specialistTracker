...
export default function PuntCoachesChartPage() {
  ...
  const { athletes, history } = usePunt();
    ...
    <main className="p-4 lg:p-6 max-w-lg mx-auto space-y-4">
      ...
      {isCoach && (saved ? (
        <div className="space-y-4 text-center py-8">
          ...
          <p className="text-sm text-muted">{reps} punts — {selectedPlayers.length} athlete{selectedPlayers.length !== 1 ? "s" : ""}</p>
        ...
        <>
          ...
          <button onClick={handleAssign} disabled={!parseInt(reps) || selectedPlayers.length === 0 || !dueDate} className="btn-primary w-full py-3 text-sm font-bold disabled:opacity-40">
            Assign Chart ({reps} punts → {selectedPlayers.length} athlete{selectedPlayers.length !== 1 ? "s" : ""})
          </button>
        </>
      ))}
