...
export default function LongSnapPuntSessionPage() {
  ...
  // Stats filtered to this snap type
  ...
  const handleAddSnap = (snap: LongSnapEntry) => {
    setSessionSnaps((prev) => { const next = [...prev, snap]; saveDraftLocal(next); return next; });
  };
      ...
      <div className="lg:w-[55%] flex flex-col border-b lg:border-b-0 lg:border-r border-border min-h-0">
        ...
        <div className="overflow-y-auto border-b border-border">
          <div className={viewOnly ? "pointer-events-none opacity-60" : ""}>
            <SnapEntryCard athletes={athleteNames} snapCount={sessionSnaps.length} onAdd={handleAddSnap} lockedType={SNAP_TYPE} />
          </div>
        </div>
