...
export function AthleteSnapPopup({ snapType, athletes, holders: holdersProp, holderEnabled = true, hideHolderPicker = false, kickerName, kickDistance, kickHash, previousSnaps, initialSnapTime, onClose, onSaved, onHolderToggle, kickList, onKickSelect }: Props) {
  ...
  // Team mode (kickList present): a kick holds one snap. If it already has one,
  // the popup pre-fills it and saving updates that snap in place (see parent).
  const isEditing = !!kickList && kickList.length > 0 && (previousSnaps?.length ?? 0) > 0;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-surface border border-border rounded-xl w-full max-w-sm mx-4 p-4 space-y-3 max-h-[90vh] overflow-y-auto">
        ...
        <button onClick={handleSave} disabled={!canSave} className="btn-primary w-full py-2 text-sm font-bold disabled:opacity-40">
          {isEditing ? "Update Snap" : "Log Snap"}
        </button>
